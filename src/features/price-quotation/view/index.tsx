...
import QutTable from "./table";

function PriceQuotationView() {
  const [update] = useUpdatePriceQuotationMutation();
  ...
  const onCellValueChanged = async ({
    ...
    try {
      ...
      const items = quotation.item_data.items.map((item) => {
        if (item._id == id) {
          return { ...item, [field]: newValue };
        }
        return item;
      });
      const formData = { quotationId: quotation.item_data.invoiceId, items };

      const { error: updateError } = await update({ formData });
  ...
  const quotationId = activePage.id as string;
  const query = useAppSelector((state) => state.priceQut.query);
  const { refetch: refetchQuotation } = useGetPriceQuotationsQuery(query);
  ...
  invoiceConfig.labels.subTitle = "Embroidery Price Quotation";
  invoiceConfig.labels.invoiceNoPrefix = "Quot No";
  ...
  const reactToPrintFn = useReactToPrint({
    ...
    documentTitle: quotation?.invoiceNo || "PriceQuotation",
    bodyClass: "m-0 p-0",
  });
  if (!quotation) {
    return <>Loading...</>;
                ...
                <div className="flex justify-between leading-5">
                  ...
                  <div className="">
                    <p className="text-black">
                      Date:{" "}
                      <b>
                        {format(quotation?.date || new Date(), "dd-MMM-yyyy")}
                      </b>
                    </p>
                    <p className="text-black">
                      {invoiceConfig.labels.invoiceNoPrefix}:{" "}
                      <b> {quotation?.invoiceNo}</b>{" "}
                    </p>
                  </div>
                </div>
                ...
                <Row sx={{ justifyContent: "space-between", mt: 2 }}>
                  <Box>
                    <Typography fontSize={10} className="text-black">
                      Note : {'"USD 1.00"'} = {'"BDT'}
                      {numberWithCommas(
                        quotation?.item_data.items[0].exchange_rate,
                      )}
                      {'"'}
                    </Typography>
                    <Typography className="text-black">
                      In Word :{toWords(quotation?.amount || 0)}
                    </Typography>
                  ...
                  {showQr && (
                    <Box>
                      <QRCode
                        bgColor={"transparent"}
                        value={`/view/qutation/${quotation?._id}`}
                        size={80}
                      />
                    </Box>
                  )}
...
export default PriceQuotationView;
