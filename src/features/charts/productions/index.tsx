...
import { useProductionChart } from "./useProductionChart";
import { useMenu } from "@/hooks/useMenu";
import { Icons } from "@/components/icons";
import { ProductionsChartQueryType } from "@/types/production";

function ProductionsChart({
  queryParams,
}: {
  queryParams?: Partial<ProductionsChartQueryType>;
}) {
  const { isLoading, options, query, handleQueryChange } =
    useProductionChart(queryParams);
    ...
    <Box>
      <Paper sx={{ border: 1, borderColor: "divider" }}>
        <ToolBarStyled sx={{ border: 0 }}>
          <Typography variant="subtitle1">Productions</Typography>
          <Stack
            spacing={1}
            direction={"row"}
